...
const loginUser = asyncHandler(async (req, res) => {
  const { email, password, loginAs = "user" } = req.body;
  const authMode = loginAs.toLowerCase();
  ...
  if (authMode === "admin" && !user.isAdmin) {
    return res.status(401).json({
      status: false,
      message: "Admin login is only for admin accounts.",
    });
  }

  if (authMode === "user" && user.isAdmin) {
    return res.status(401).json({
      status: false,
      message: "This account is an admin account. Please use admin login.",
    });
  }
